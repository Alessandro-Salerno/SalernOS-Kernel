...
static void __combine_forward__(heapseghdr_t *__segment) {
  ...
  if (__segment->_Next == lastHeader)
    lastHeader = __segment;

  if (__segment->_Next->_Next != NULL)
    __segment->_Next->_Next->_Last = __segment;

  __segment->_Length =
      __segment->_Length + __segment->_Next->_Length + sizeof(heapseghdr_t);
...
static heapseghdr_t *__split__(heapseghdr_t *__segment, size_t __size) {
  ...
  heapseghdr_t *_split_hdr =
      (heapseghdr_t *)((size_t)(__segment) + __size + sizeof(heapseghdr_t));
  ...
  if (lastHeader == __segment)
    lastHeader = _split_hdr;
  return _split_hdr;
}
