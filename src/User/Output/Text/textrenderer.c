...
static void __initialize__(uint32_t __color,
                           uint32_t __backcolor,
                           uint64_t __xoff,
                           uint64_t __yoff) {
  ...
  startPosition = (point_t){.x = __xoff, .y = __yoff};

  endPosition =
      (point_t){.x = _buff_w - (_buff_w % 8), .y = _buff_h - (_buff_h % 16)};
...
void kernel_text_scroll(uint32_t __lines) {
  ...
  for (uint64_t _y = __lines * 16; _y < endPosition.y; _y++) {
    for (uint64_t _x = startPosition.x; _x < kernel_kdd_fbo_get()._Width;
         _x++) {
      kernel_kdd_pxcolor_set(
          _x, _y - __lines * 16, kernel_kdd_pxcolor_get(_x, _y));
    }
  }
...
void kernel_text_line_clear(uint32_t __line) {
  ...
  for (uint64_t _y = __line * 16 - 16; _y < __line * 16; _y++) {
    for (uint64_t _x = startPosition.x; _x < endPosition.x; _x++) {
      kernel_kdd_pxcolor_set(_x, _y, backgroundColor);
    }
  }
}
...
void kernel_text_info_set(uint32_t __color,
                          uint32_t __backcolor,
                          point_t  __pos) {
  SOFTASSERT(trendrInitialized, RETVOID);
...
void kernel_text_initialize(uint32_t  __color,
                            uint32_t  __backcolor,
                            uint32_t  __xoff,
                            uint32_t  __yoff,
                            bmpfont_t __font) {
...
void kernel_text_reinitialize(uint32_t __color,
                              uint32_t __backcolor,
                              uint32_t __xoff,
                              uint32_t __yoff) {
  SOFTASSERT(trendrInitialized, RETVOID);
...
void kernel_text_info_get(uint32_t  *__color,
                          uint32_t  *__backcolor,
                          point_t   *__curpos,
                          point_t   *__lpos,
                          bmpfont_t *__font) {
