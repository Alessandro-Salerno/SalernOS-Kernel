...
static void __reserve_page__(void *__address) {
  panic_assert(pgfInitialized >= 2, PGFALLOC_FAULT);

  uint64_t _idx = (uint64_t)(__address) / 4096;

  SOFTASSERT(!(bmp_get(&pgBitmap, _idx)), RETVOID)

  bmp_set(&pgBitmap, _idx, 1);
  mFreeSize -= 4096;
  mReservedSize += 4096;
}
...
static void __init__bitmap__() {
  panic_assert(pgfInitialized == 1, PGFALLOC_DBMP);
...
void pgfa_initialize() {
  ...
  memseg_t  _bmp_seg;
  meminfo_t _meminfo;
  mmap_info_get(&mFreeSize, NULL, &_bmp_seg, &_meminfo);
  mSegment = _bmp_seg;

  pgfInitialized = TRUE;
  __init__bitmap__();

  pgfa_reserve(0, mFreeSize / 4096 + 1);

  for (uint64_t _i = 0; _i < _meminfo._MemoryMapSize / _meminfo._DescriptorSize;
       _i++) {
    efimemdesc_t *_mem_desc = mmap_entry_get(_i);

    if (_mem_desc->_Type == USABLE_MEM_TYPE)
      pgfa_unreserve(_mem_desc->_PhysicalAddress, _mem_desc->_Pages);
  }

  pgfa_reserve(0, 0x100);
  pgfa_lock(pgBitmap._Buffer, pgBitmap._Size / 4096 + 1);
  pgfInitialized = 3;
}
...
void *pgfa_page_new() {
  ...
  for (; pgBmpIndex < pgBitmap._Size * 8; pgBmpIndex++) {
    if (bmp_get(&pgBitmap, pgBmpIndex) == 0) {
