...
static inline void profiler_calc_elapsed_time(uintmax_t *out_real_time,
    ...
    uintmax_t     end_real     = ARCH_CPU_GET_TIME();
...
static inline com_profiler_data_t profiler_init_data(void) {
    com_profiler_data_t data        = {.real_start = ARCH_CPU_GET_TIME(), 0};
    com_thread_t       *curr_thread = ARCH_CPU_GET_THREAD();
    if (NULL != curr_thread) {
...
void com_sys_profiler_end_function(com_profiler_data_t *data) {
    ...
    com_profile_func_data_t *func_data = &SystemProfile
                                              .functions[data->function_id];
    __atomic_add_fetch(&func_data->num_calls, 1, __ATOMIC_SEQ_CST);
    __atomic_add_fetch(&func_data->real_time, real_elapsed, __ATOMIC_SEQ_CST);
    __atomic_add_fetch(&func_data->cpu_time, cpu_elapsed, __ATOMIC_SEQ_CST);
}
...
void com_sys_profiler_end_syscall(com_profiler_data_t *data) {
    ...
    __atomic_add_fetch(&syscall_data->num_calls, 1, __ATOMIC_SEQ_CST);
    __atomic_add_fetch(&syscall_data->real_time,
                       real_elapsed,
                       __ATOMIC_SEQ_CST);
    __atomic_add_fetch(&syscall_data->cpu_time, cpu_elapsed, __ATOMIC_SEQ_CST);
}
